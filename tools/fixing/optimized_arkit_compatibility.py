...
确保USDZ文件与Apple官方示例完全一致：
1. 保持diffuseColor（不转换为baseColor）
2. 设置正确的单位（适合分子结构）
3. 添加缺失的法线数据
4. 确保正确的可见性和用途设置
5. 移除不兼容的材质属性
...
class OptimizedARKitCompatibilityFixer:
    """优化的ARKit兼容性修复器"""
    ...
    def fix_usdz_file(self, usdz_path: str) -> bool:
        ...
        logger.info(f"开始优化ARKit兼容性: {usdz_path}")
        ...
        try:
            ...
            # 2. 确保材质使用diffuseColor（移除baseColor）
            self._ensure_diffuse_color_materials(stage)
            ...
            # 5. 移除不兼容的材质属性
            self._remove_incompatible_material_properties(stage)
            
            # 保存修复后的文件
            success = self._save_stage(stage, usdz_path)
            
            if success:
                logger.info(f"优化ARKit兼容性完成，应用了 {len(self.fixes_applied)} 个修复")
            ...
            logger.error(f"优化ARKit兼容性时出错: {e}")
            return False
    ...
    def _ensure_diffuse_color_materials(self, stage: Usd.Stage):
        """确保材质使用diffuseColor而非baseColor（符合官方示例）"""
        try:
            materials_fixed = 0
                        ...
                        if connected_source and len(connected_source) >= 2:
                            ...
                            if shader_prim and shader_prim.IsValid():
                                ...
                                if base_color_input and base_color_input.HasValue():
                                    ...
                                    logger.info(f"  发现baseColor: {base_color_value}，需要转换为diffuseColor")
                                    ...
                                    if base_color_value is not None:
                                        ...
                                        materials_fixed += 1
                                        self.fixes_applied.append(f"材质 {prim.GetName()} 从baseColor转换为diffuseColor")
                                
                                elif diffuse_input and diffuse_input.HasValue():
                                    # 已经有diffuseColor，确保没有baseColor
                                    if base_color_input:
                                        shader_prim.RemoveProperty('inputs:baseColor')
                                        logger.info(f"  ✓ 移除多余的baseColor属性")
                                        materials_fixed += 1
                                        self.fixes_applied.append(f"材质 {prim.GetName()} 移除多余的baseColor")
                                    
                                    diffuse_value = diffuse_input.Get()
                                    logger.info(f"  ✓ 材质已使用diffuseColor: {diffuse_value}")
                                
                                else:
                                    # 既没有baseColor也没有diffuseColor，创建默认的diffuseColor
                                    logger.info(f"  材质缺少颜色属性，创建默认diffuseColor")
                                    if not diffuse_input:
                                        diffuse_input = shader.CreateInput('diffuseColor', Sdf.ValueTypeNames.Color3f)
                                    diffuse_input.Set(Gf.Vec3f(0.8, 0.8, 0.8))  # 默认灰色
                                    materials_fixed += 1
                                    self.fixes_applied.append(f"材质 {prim.GetName()} 添加默认diffuseColor")
            
            if materials_fixed > 0:
                logger.info(f"✓ 修复了 {materials_fixed} 个材质，确保使用diffuseColor")
            else:
                logger.info("✓ 所有材质已正确使用diffuseColor属性")
                
        except Exception as e:
            error_msg = f"确保diffuseColor材质失败: {e}"
            logger.error(error_msg)
            self.errors.append(error_msg)
    
    def _remove_incompatible_material_properties(self, stage: Usd.Stage):
        """移除不兼容的材质属性"""
        try:
                    ...
                    if surface_output:
                        ...
                        if connected_source and len(connected_source) >= 2:
                            ...
                            if shader_prim and shader_prim.IsValid():
                                ...
                                # 检查并移除不兼容的属性
                                for prop_name in incompatible_properties:
                                    if shader_prim.HasProperty(prop_name):
                                        shader_prim.RemoveProperty(prop_name)
                                        logger.info(f"  移除不兼容属性: {prop_name} from {prim.GetName()}")
                                        properties_removed += 1
                                        self.fixes_applied.append(f"移除 {prim.GetName()} 的 {prop_name}")
            
            if properties_removed > 0:
                logger.info(f"✓ 移除了 {properties_removed} 个不兼容的材质属性")
            else:
                logger.info("✓ 没有发现不兼容的材质属性")
                
        except Exception as e:
            error_msg = f"移除不兼容材质属性失败: {e}"
            logger.error(error_msg)
            self.errors.append(error_msg)
    ...
    def _fix_visibility_and_purpose(self, stage: Usd.Stage):
        ...
        try:
            ...
            for prim in stage.Traverse():
                if prim.IsA(UsdGeom.Imageable):
                    ...
                    if not visibility_attr or visibility_attr.Get() != UsdGeom.Tokens.inherited:
                        ...
                        prims_fixed += 1
                        self.fixes_applied.append(f"设置 {prim.GetName()} 可见性为inherited")
                        logger.info(f"  ✓ 设置 {prim.GetPath()} 可见性为inherited")
            ...
            if prims_fixed > 0:
                logger.info(f"✓ 修复了 {prims_fixed} 个图元的可见性和用途设置")
            else:
                logger.info("✓ 所有图元的可见性和用途设置正确")
    ...
    def _save_stage(self, stage: Usd.Stage, usdz_path: str) -> bool:
        ...
        try:
            ...
            if success:
                logger.info(f"成功保存优化后的USDZ文件: {usdz_path}")
                return True
            else:
...
def main():
    """主函数"""
    if len(sys.argv) != 2:
        print("用法: python optimized_arkit_compatibility.py <usdz_file>")
        print("")
        print("功能:")
        print("  - 确保材质使用diffuseColor（符合官方示例）")
        print("  - 设置正确的单位（适合分子结构）")
        print("  - 添加缺失的法线数据")
        print("  - 确保正确的可见性和用途设置")
        print("  - 移除不兼容的材质属性")
        sys.exit(1)
    ...
    # 创建修复器并执行修复
    fixer = OptimizedARKitCompatibilityFixer()
    success = fixer.fix_usdz_file(usdz_file)
    
    if success:
        logger.info("✅ 优化ARKit兼容性成功")
        logger.info("")
        logger.info("修复内容:")
        logger.info("  ✓ 确保材质使用diffuseColor")
        logger.info("  ✓ 设置正确的单位")
        logger.info("  ✓ 修复法线数据")
        logger.info("  ✓ 修复可见性和用途")
        logger.info("  ✓ 移除不兼容属性")
